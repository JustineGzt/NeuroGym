...
import { Card } from './ui/Card'; // Attention au chemin vers ton dossier UI !
    ...
    <Card className={`mb-3 transition-all duration-300 ${completed ? 'bg-[#2a9d8f]/5 border-[#2a9d8f]/20' : 'bg-white'}`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h4 className={`font-bold text-base ${completed ? 'text-gray-400 line-through' : 'text-[#1d3557]'}`}>
              {title}
            </h4>
            <p className="text-xs font-semibold text-orange-400 mt-0.5">🔥 {streak} day streak</p>
          </div>
        </div>
        <button 
